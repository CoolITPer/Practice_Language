...
1.创建udp
2.绑定端口
3.udp_socket.sendto(data.encode(), ('169.254.55.58', 8080))
4.recv_data, remote_address = udp_socket.recvfrom(4096)
'''
...
while True:
    ...
    udp_socket.sendto(data.encode(), ('169.254.55.58', 8080))

    # 3 接收数据　recvfrom　会阻塞等待对方发送数据----参数表示本次接收数据的最大长度
    # 返回值就是接收数据的相关信息  (b'hehehe', ('169.254.55.58', 8080))
